...
const countDownDate = 1661972400000;
...
function App() {
  ...
  useEffect(() => {
    ...
    var x = setInterval(function () {
      ...
      document.getElementById("launchTime").innerHTML =
        days + hours + minutes + "mins : " + seconds + "sec";
  ...
  const btnhandler = () => {
    ...
    if (window.ethereum) {
      ...
      console.log("reqested some stuff!");
    } else {
      alert("install metamask extension!!");
    }
  };
  ...
  const postRequestMerkle = async (wallet) => {
    ...
    var merkleTree = new MerkleTree(wlg.map(hashAccount), keccak256, {
      sortPairs: true,
    });
    var merkleProof = merkleTree.getHexProof(hashAccount(wallet));
    console.log("gold hexRoot: ", merkleTree.getHexRoot());
    if (
      merkleTree.verify(merkleProof, keccak256(wallet), merkleTree.getHexRoot())
    ) {
      return [merkleProof, 1];
    } else {
      merkleTree = new MerkleTree(wlt.map(hashAccount), keccak256, {
        sortPairs: true,
      });
      merkleProof = merkleTree.getHexProof(hashAccount(wallet));
      console.log("team hexRoot: ", merkleTree.getHexRoot());
      if (
        merkleTree.verify(
          merkleProof,
          keccak256(wallet),
        ...
        merkleTree = new MerkleTree(wlo.map(hashAccount), keccak256, {
          sortPairs: true,
        });
        merkleProof = merkleTree.getHexProof(hashAccount(wallet));
        console.log("og hexRoot: ", merkleTree.getHexRoot());
        if (
          merkleTree.verify(
            merkleProof,
            keccak256(wallet),
          ...
          merkleTree = new MerkleTree(wlf.map(hashAccount), keccak256, {
            sortPairs: true,
          });
          merkleProof = merkleTree.getHexProof(hashAccount(wallet));
          console.log("free hexRoot: ", merkleTree.getHexRoot());
          if (
            merkleTree.verify(
              merkleProof,
              keccak256(wallet),
            ...
            merkleTree = new MerkleTree(wlw.map(hashAccount), keccak256, {
              sortPairs: true,
            });
            merkleProof = merkleTree.getHexProof(hashAccount(wallet));
            console.log("wl hexRoot: ", merkleTree.getHexRoot());
            if (
              merkleTree.verify(
                merkleProof,
                keccak256(wallet),
  ...
  const mintNFT = async () => {
    ...
    console.log("now - public_start: ", now - public_start);

    console.log(
      "174 window.ethereum.selectedAddress: ",
    ...
    try {
      const receipt = await SmartContractObj.methods
        .mint(mintAmount, merkleProof)
        .send({
      ...
      return {
        success: false,
        status:
          "😥 Something went wrong: Did not obtain merkle proof, status 567",
      };
    }
  };
  ...
  return (
    <ChakraProvider>
      <Flex
        ...
        w="100%"
        direction="column"
        bg="linear-gradient(290deg, rgb(20,20,50), rgb(40,40,120))"
        color="#fff"
        fontFamily="chalk"
        px={["4%", "6%"]}
        pt={["4%", "2.8%"]}
      ...
      >
        ...
        >
          <Flex align="center">
            ...
            <Text
              ...
              ml="3"
              display={["none", "block"]}
              textShadow="6px 2px rgb(70,70,70)"
              fontSize={["18px", "24px"]}
            >
              RDH Academy
            </Text>
          ...
          <Flex flex="1" align="center" justify="flex-end">
            ...
            <Flex
              ...
              mr={["3", "5"]}
              onClick={() =>
                window.open("https://twitter.com/RDHNFTs?s=21", "_BLANK")
              }
              color="#1DA1F2"
            >
            ...
            <Flex
              ...
              mr={["3", "5"]}
              onClick={() =>
                window.open("https://www.instagram.com/rdhnfts/", "_BLANK")
              }
            >
            ...
            <Flex
              ...
              color="#7289da"
              onClick={() =>
                window.open("https://discord.gg/DZevtsgaht", "_BLANK")
              }
            >
          ...
          <Flex w={["100%", "45%"]} direction="column">
            {funcTrue && (
              <Box
                ...
                bg="rgba(14, 232, 14, .2)"
              >
                Minted!! Check out your transaction on{" "}
                <Text
                  textDecoration="underline"
                  fontWeight="bold"
                  cursor="pointer"
                  onClick={() => window.open(ETHERSCAN_LINK)}
                >
                  {ETHERSCAN_LINK}
                </Text>
              </Box>
            ...
            >
              ...
              <Box fontSize={["16px", "17px"]}>
                <Text
                  ...
                  bg="rgb(80,80,130)"
                  transition="300ms ease-in-out"
                  _hover={{ bg: "rgb(100,100,150)" }}
                  data-aos="fade-up"
                  color="rgb(255, 0, 98)"
                >
                  <i className="mdi mdi-human-greeting"></i>
                </Text>
                <Text color="rgb(200,200,250)" mb="1" id="rdMap">
                  Rekted diamond hands(RDH)
                </Text>
                Is a collection for the community and by the community, 5555
                ERC-721 tokens randomly generated on the ethereum blockchain. We
                aim to be a source of light for rekt hodlers in the dark tunnel
                of the bear season, FOMO in to get your rekted bags pumped.
              </Box>
            </Flex>
            ...
            >
              <Box fontSize={["16px", "17px"]}>
                ...
                <Text color="rgb(200,200,250)" mb="1">
                  Roadmap
                </Text>
                Only roadmap we have is to keep building based on hodlers
                interaction and incorporate feedback from our community.
              </Box>
            </Flex>
          </Flex>
        ...
        >
          ...
          >
            ...
            <Text
              ...
              data-aos="fade-left"
            >
              All items in this collection will represent the experience a
              holder encountered in his/her Cypto/NFT journey.
              <br />
              ...
              >
                ...
                >
                  <Text
                    ...
                    mb="2"
                    onClick={() =>
                      window.open("https://twitter.com/tulipmaneth", "_BLANK")
                    }
                  >
                    @Tulipmaneth
                  </Text>
                  Also known as Tulipman in the community, Tulipman is a UAE
                  based Full-stack developer and NFT consultant
                </Box>
              </Flex>
              ...
              >
                ...
                >
                  <Text
                    ...
                    mb="2"
                    onClick={() =>
                      window.open("https://twitter.com/mastrodre", "_BLANK")
                    }
                  >
                    @Mastrodre
                  </Text>
                  Also known as OG kelvin is a Dubai based web 3 enthusiast and
                  investor.
                </Box>
              </Flex>
            </Flex>
                  ...
                  <Flex direction="column" align="center" w={["100%", "50%"]}>
                    ...
                    >
                      <Text
                        ...
                        onClick={() =>
                          window.open(
                            "https://twitter.com/thelunalabs?s=11&t=26eD2Z6tX4ktKWIeAkj7kg",
                            "_BLANK"
                          )
                        }
                      >
        ...
        >
          <Text textAlign="center" data-aos="fade-up">
            Created by the RDH founders with special love to our community, Our
            friends, families, and loved ones
          </Text>
          <Box textAlign="center" mt="3" data-aos="fade-up">
            For all press inquiries, please contact{" "}
            <Text as="span" color="rgb(200,200,250)" mb="2">
              press@rdh.club
            </Text>
          </Box>
          ...
          >
            ...
            <Flex
              ...
              mr={["3", "5"]}
              cursor="pointer"
              onClick={() => window.open("https://discord.gg/rdhnft", "_BLANK")}
            >
              <i className="mdi mdi-discord"></i>
            </Flex>
